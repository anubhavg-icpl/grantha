...
export class ComponentPreloader {
	...
	constructor(private loaders: Record<string, ReturnType<typeof createLazyLoader>>) {}
	...
	preloadOnIdle(): void {
		if ('requestIdleCallback' in window) {
			requestIdleCallback(() => {
				const componentsToPreload = ['ChatArea', 'ModelSelector', 'ProcessedProjects'];
...
export class LazyLoadPerformanceMonitor {
	...
	getAverageLoadTime(): number {
		const times = Array.from(this.loadTimes.values()).filter(time => typeof time === 'number');
