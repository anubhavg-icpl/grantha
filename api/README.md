...
This is the backend API for Grantha, providing smart knowledge management and AI-powered documentation generation.
...
Grantha supports multiple LLM providers. The environment variables above are required depending on which providers you want to use:
...
##### Configuration Files
Grantha now uses JSON configuration files to manage various system components instead of hardcoded values:
...
```
GRANTHA_CONFIG_DIR=/path/to/custom/config/dir  # Optional, for custom config file location
```
